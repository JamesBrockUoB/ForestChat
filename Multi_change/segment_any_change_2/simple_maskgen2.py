...
class SimpleMaskGenerator:
    ...
    def generate_with_image_embedding(self, image_embedding, original_size):
        ...
        for (points,) in batch_iterator(self.points_per_batch, points_for_image):
            batch_data = self._process_batch(
                image_embedding, points, original_size, crop_box, original_size
            )
    ...
    def generate(self, image, mask_output_mode="rle"):
        ...
        if mask_output_mode == "rle":
            ...
            data["segmentations"] = np.stack(
                [rle_to_mask(rle) for rle in data["rles"]], axis=0
            )
        else:
            raise ValueError
        ...
        image_embedding = F.interpolate(
            image_embedding, size=orig_size, mode="bilinear", align_corners=True
        )
        image_embedding = image_embedding.squeeze_(0)
        return {"mask_data": data, "image_embedding": image_embedding}
