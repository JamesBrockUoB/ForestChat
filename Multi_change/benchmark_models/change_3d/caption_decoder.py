...
class MCCFormers_diff_as_Q(nn.Module):
    ...
    def __init__(self, feature_dim, dropout, h, w, d_model=512, n_head=4, n_layers=3):
        ...
        self.LN = nn.ModuleList([nn.LayerNorm(d_model * 2) for _ in range(n_layers)])
    ...
    def forward(self, img_feat1, img_feat2):
        ...
        position_embedding = (
            position_embedding.permute(2, 0, 1).unsqueeze(0).repeat(batch, 1, 1, 1)
        )
...
class Mesh_TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.multihead_attn2 = nn.MultiheadAttention(
            d_model, int(nhead), dropout=dropout
        )
        self.multihead_attn3 = nn.MultiheadAttention(
            int(d_model), int(nhead), dropout=dropout
        )
