...
from change3d.caption_decoder import CaptionDecoder
from change3d.change_decoder import ChangeDecoder
from change3d.utils import weight_init
from change3d.x3d import create_x3d
...
class Encoder(nn.Module):
    ...
    def base_forward(
        self, x: torch.Tensor, output_final: bool = False
    ) -> List[torch.Tensor]:
        """
    ...
    def forward(
        self, x: torch.Tensor, y: torch.Tensor, output_final: bool = False
    ) -> List[torch.Tensor]:
        """
        ...
        expand_percep_frames = repeat(
            self.perception_frames, "1 c t h w -> b c t h w", b=x.shape[0]
        )

        # Combine into 3-frame sequence [input, token, target]
        frames = torch.cat(
            [x.unsqueeze(2), expand_percep_frames, y.unsqueeze(2)], dim=2
        )
...
class Trainer(nn.Module):
    ...
    def __init__(self, args: Any) -> None:
        ...
        self.decoder_cd = ChangeDecoder(args, in_dim=self.embed_dims, has_sigmoid=True)
        # Initialize decoder weights
        weight_init(self.decoder_cd)
